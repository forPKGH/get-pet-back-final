...
const EmailChangable = ({ valueProps }) => {
  ...
  return (<>
    <form ref={form} onSubmit={changeEmail} className="row g-3 needs-validation" noValidate>
      <div className="input-group w-100">
        <input required ref={input} onChange={handleChange} style={{ borderRadius: '0px' }} className="form-control list-group-item" type="email" value={value} />
        <button type='submit' style={{ borderRadius: '0px' }} className="input-group-text" id="basic-addon1">{value === oldValue ? '✏️' : '✅'}</button>
        <div className="invalid-feedback">
          Неверный формат email или email занят
        </div>
      </div>
    </form>
    {
